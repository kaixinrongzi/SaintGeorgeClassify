...
## Experiments
### Overfitting Issue
During the training of all three models (ResNet, EfficientNet, and ViT), I encountered severe overfitting, with the models performing well on training data but poorly on validation data. To address this, I implemented several techniques aimed at improving generalization and preventing overfitting:
...
These strategies collectively contributed to more stable training and improved validation performance, mitigating overfitting across all models.

### Training Time Consumption
Given the computational resources and project timeline constraints, it was essential to balance model performance with training efficiency. Training deep learning models, especially with multiple architectures like ResNet, EfficientNet, and ViT, can be time-consuming, particularly for a large volume of input. Therefore, I implemented **early stopping** based on validation performance to avoid unnecessary training. This approach ensured efficient use of resources while maximizing model effectiveness, enabling rapid experimentation and iteration without unnecessary computational overhead.
